<template>
  <div>
    <el-card shadow="hover">
      <div id="ProductTypeClickChart" :style="{width: '100%', height: '300px'}"></div>
    ...
    export default {
        ...
        methods:{
            ...
            drawLine(title,names,data) {//实例化产品图形
                ...
                let myChart = this.$echarts.init(document.getElementById('ProductTypeClickChart'),"fresh-cut");
                let option = {
                    title: {
                        text: title,
                        subtext: '即时数据',
            ...
            getData(){
                this.$axios
                    .get("/api/backstage/count/maxClickOfProductType",{
                        params: {
                            limit:10
                        }
                    })
                    .then(response => {//获取返回数据
                        ...
                        if (msg.code === 0) {
                            ...
                            this.drawLine("各产品栏目关注度比重",names,msg.data);
                        }
                    });
            }
        },
