    ...
    export default {
        ...
        methods:{
            drawLine(title,productNames,productSalesVolumes) {//实例化产品图形
                ...
                let myChart = this.$echarts.init(document.getElementById('productChart'),"blue")
                // 绘制图表
                myChart.setOption({
                    title: {text: title},
                    tooltip: {},
            ...
            getData(){
                this.$axios
                    .get("/api/backstage/count/maxSalesVolumeOfProduct",{
                        params: {
                            limit:5
                        }
                    })
                    .then(response => {//获取返回数据
                        ...
                        if (msg.code === 0) {
                            this.drawLine("商品销量排行榜",msg.data["productNames"],msg.data["productSalesVolumes"]);
                        }
                    });
            }
        },
